...
const Exhibit = ({exhibit}) => {
    ...
    return (
        <>{exhibit.pages.map((page) => {
            if (page.order === 0) {
                return (<section className={scss.exhibit__section}><Title exhibit={exhibit}/></section>)
            } else {
                return (<section className={scss.exhibit__section}><Chapter chapter={page}/></section>)
            }
        })}</>
    )
